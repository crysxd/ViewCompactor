...
class ChangeTextSizeTransform : Transition() {
    ...
    private fun captureValues(transitionValues: TransitionValues) {
        if (transitionValues.view is TextView) {
            ...
            transitionValues.values[PROP_NAME_TEXT_SIZE] = textView.textSize
        }
    }
    ...
    override fun createAnimator(
        ...
        val startSize = startValues?.values?.get(PROP_NAME_TEXT_SIZE) as Float?
        val endSize = endValues?.values?.get(PROP_NAME_TEXT_SIZE) as Float?
    ...
    companion object {
        private val PROP_NAME_TEXT_SIZE: String? = "alexjlockwood:transition:textsize"
